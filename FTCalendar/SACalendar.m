...
{
    if (nil != _delegate && [_delegate respondsToSelector:@selector(didDisplayCalendarForMonth:year:)])
        [_delegate didDisplayCalendarForMonth:month year:year];
}
...
- (UIViewController *) controllerAtIndex:(NSInteger) index {
    ...
    if (![controllers objectForKey:[NSString stringWithFormat:@"%li",(long)index]]) {
        ...
        UILabel *monthLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44)];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!(indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth)) {
        ...
        if (nil != _delegate && [_delegate respondsToSelector:@selector(didSelectDate:month:year:)])
        {
